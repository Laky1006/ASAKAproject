...
<template>
  <div class="backdrop-blur-lg bg-white/40 rounded-2xl shadow-2xl border border-white/60 overflow-hidden font-heading">
      ...
      <table class="min-w-full divide-y divide-white/30">
        <thead class="bg-white/30 backdrop-blur-sm">
          <tr>
            ...
            <th class="px-4 sm:px-6 py-3 text-left text-xs font-bold text-[#2D1810] uppercase tracking-wider">Title</th>
            <th class="px-4 sm:px-6 py-3 text-left text-xs font-bold text-[#2D1810] uppercase tracking-wider">Provider</th>
            <th class="px-4 sm:px-6 py-3 text-left text-xs font-bold text-[#2D1810] uppercase tracking-wider">Rating</th>
        ...
        <tbody class="divide-y divide-white/20 font-body">
          <tr v-for="s in servicesList" :key="s.id" class="hover:bg-white/30 transition-colors duration-150">
            <td class="px-4 sm:px-6 py-3 text-sm text-[#6b5b73] font-medium">{{ s.id }}</td>
            <td class="px-4 sm:px-6 py-3 text-sm font-semibold text-[#2D1810]">{{ s.title }}</td>
            <td class="px-4 sm:px-6 py-3 text-sm text-[#6b5b73]">{{ s.provider?.name || 'Unknown' }}</td>
            <td class="px-4 sm:px-6 py-3 text-sm text-[#6b5b73]">{{ s.rating ?? '—' }}</td>
          ...
          <tr v-if="!services?.data?.length">
            <td colspan="6" class="px-6 py-12 text-center text-[#6b5b73] font-body">No services found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- Pagination -->
    <div class="p-4 flex flex-wrap gap-2 bg-white/20 backdrop-blur-sm border-t border-white/30" v-if="services?.links">
      <Link
